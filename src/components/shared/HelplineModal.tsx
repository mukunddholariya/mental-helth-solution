...
import { STUDENT_HELPLINES } from "@manas/core";
...
export default function HelplineModal({ isOpen, onClose }: HelplineModalProps) {
  ...
  return (
    <div 
      className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 select-none animate-fadeIn" 
      id="helpline-modal-backdrop"
      role="dialog"
      aria-modal="true"
      aria-labelledby="helpline-dialog-title"
    >
      <div className="w-full max-w-lg bg-slate-900 border border-slate-700 rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden" id="helpline-modal-card">
        ...
        <div className="absolute top-0 right-0 w-32 h-32 bg-rose-500/10 rounded-full blur-2xl pointer-events-none" aria-hidden="true"></div>
        ...
        <div className="flex items-start justify-between border-b border-slate-800 pb-4" id="helpline-modal-header">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 id="helpline-dialog-title" className="text-lg font-bold text-white font-display">Mental Wellness Helplines India</h3>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="my-5 p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl" id="helpline-reassurance">
          <div className="flex gap-2.5">
            <Heart className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" aria-hidden="true" />
        ...
        <div className="space-y-4" id="helplines-content-list" role="list">
          {STUDENT_HELPLINES.map((h) => (
            <div 
              key={h.number} 
              className="bg-slate-950 border border-slate-800 hover:border-slate-700/60 transition p-4 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3" 
              id={`helpline-modal-item-${h.number}`}
              role="listitem"
            >
        ...
        <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between text-[11px] text-slate-500" id="helpline-modal-footer">
          <span className="flex items-center gap-1">
            <Lock className="w-3.5 h-3.5 shrink-0 text-emerald-400/60" aria-hidden="true" />
